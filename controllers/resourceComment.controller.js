...
async function create(req, res) {
  ...
  const exists = await Resource.findOne(scoped(req.user._id, { _id: resource }));
...
async function list(req, res) {
  ...
  const visible = await Resource.findOne(scoped(req.user._id, { _id: resource }));
  if (!visible) throw ApiError.notFound("That resource doesn't exist");
